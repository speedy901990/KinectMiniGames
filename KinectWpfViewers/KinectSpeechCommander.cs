...
namespace Microsoft.Samples.Kinect.WpfViewers
{
    ...
    public class KinectSpeechCommander : Control, IDisposable
    {
        ...
        public void Start(KinectSensor sensor)
        {
            ...
            defaultListeningGrammar = CreateGrammar(DefaultListeningGrammarUri, DefaultListeningGrammarRule);
            activeListeningGrammar = CreateGrammar(ActiveListeningGrammarUri, ActiveListeningGrammarRule);

            // If we couldn't find either of the grammars, we can't do any work, so we bail out
            if (defaultListeningGrammar == null || activeListeningGrammar == null)
            {
                return;
            }

            kinect = sensor;
            activeListeningTimer = new DispatcherTimer();
            ...
            speechRecognitionEngine.LoadGrammar(defaultListeningGrammar);
            speechRecognitionEngine.LoadGrammar(activeListeningGrammar);

            EnableGrammars();

            // Hook - up speech recognition events.
            speechRecognitionEngine.SpeechRecognized += Sre_SpeechRecognized;
            speechRecognitionEngine.SpeechRecognitionRejected += Sre_SpeechRejected;

            // Diagnostic event to tune up the kinect mic volume
            speechRecognitionEngine.AudioSignalProblemOccurred += Sre_AudioSignalProblemOccurred;

            // Recognize now...
            speechRecognitionEngine.SetInputToAudioStream(kinect.AudioSource.Start(), new SpeechAudioFormatInfo(EncodingFormat.Pcm, 16000, 16, 1, 32000, 2, null));
        ...
        public void Stop()
        {
            ...
            if (kinect != null && kinect.AudioSource != null)
            {
        ...
        {
            if (ActiveListeningModeChanged != null)
            {
                ActiveListeningModeChangedEventArgs e = new ActiveListeningModeChangedEventArgs();
                e.ActiveListeningModeEnabled = activeListeningMode;

                ActiveListeningModeChanged(this, e);
            }
        }
        ...
        private void Sre_SpeechRecognized(object sender, SpeechRecognizedEventArgs e)
        {
            if (e.Result.Confidence > DefaultConfidenceThreshold)
            {
                ...
                OnSpeechRecognized(e.Result.Semantics.Value.ToString(), e.Result.Text, e.Result.Confidence, activeListeningMode);

                // If the keyword is used, switch to Active Listening Mode
                if (e.Result.Semantics.Value.ToString() == "KEYWORD")
                {
                    SetActiveListeningModeOn();
                }
                else
                {
                    SetActiveListeningModeOff();
                }
            }
            else
            {
                Debug.Print("Speech Rejected by confidence: {0} confidence: {1}", e.Result.Text, e.Result.Confidence);
                OnSpeechRejected(e.Result.Text, e.Result.Confidence, activeListeningMode);
            }
        }
        ...
        {
            if (activeListeningMode)
            {
                return;
            }

            activeListeningMode = true;
            OnActiveListeningModeChanged();
            EnableGrammars();
        ...
        {
            activeListeningTimer.Stop();
            activeListeningTimer.Tick += ActiveListeningTimerTick;
            activeListeningTimer.Interval = new TimeSpan(0, 0, ActiveListeningTimeout);
            activeListeningTimer.Start();
        }
        ...
        {
            if (defaultListeningGrammar != null)
            {
                defaultListeningGrammar.Enabled = !activeListeningMode;
            }

            if (activeListeningGrammar != null)
            {
                activeListeningGrammar.Enabled = activeListeningMode;
            }
        }
    }
}
